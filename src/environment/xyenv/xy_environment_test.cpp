...
public:
    void SetUp() override {
        env = std::make_unique<XYEnvironment>(10, 12);
    ...
    std::unique_ptr<XYEnvironment> env;
};
...
TEST_F(XYEnvironmentTest, testObjectIsUnique) {
    auto xy = std::make_unique<XYLocation>(5, 6);
...
TEST_F(XYEnvironmentTest, testAddObjectTwice) {
    ...
    auto xyloc = std::make_unique<XYLocation>(5, 5);
    auto xy = std::make_unique<XYLocation>(5, 5);
...
TEST_F(XYEnvironmentTest, testMoveObject) {
    ...
    auto loc54 = std::make_unique<XYLocation>(5, 4);
    auto loc64 = std::make_unique<XYLocation>(6, 4);
    auto loc65 = std::make_unique<XYLocation>(6, 5);
...
TEST_F(XYEnvironmentTest, testMoveWithBlockingWalls) {
    ...
    auto northloc = std::make_unique<XYLocation>(5, 6);
    auto southloc = std::make_unique<XYLocation>(5, 4);
    auto westloc = std::make_unique<XYLocation>(4, 5);
    auto endloc = std::make_unique<XYLocation>(6, 5);
...
TEST_F(XYEnvironmentTest, testGetSet) {
    auto loc = std::make_unique<XYLocation>(5, 7);
    env->add_to(agent.get(), *loc);
    ASSERT_EQ(env->get_set_size(*loc), size_t(1));
...
TEST_F(XYEnvironmentTest, testGetObjectsNear) {
    auto loc = std::make_unique<XYLocation>(5, 5);
    auto loc2 = std::make_unique<XYLocation>(7, 4);
    auto loc3 = std::make_unique<XYLocation>(5, 7);
    auto loc4 = std::make_unique<XYLocation>(3, 10);
    auto loc5 = std::make_unique<XYLocation>(3, 11);
...
TEST_F(XYEnvironmentTest, testOutOfRangeXYLocations) {
    auto loc = std::make_unique<XYLocation>(11, 13);
    auto loc2 = std::make_unique<XYLocation>(25, 30);
    auto loc3 = std::make_unique<XYLocation>(0, 0);
